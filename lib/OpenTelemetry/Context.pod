...
Takes a key object (see the L</key> class method) and a value and returns a
new context object with all the values of the calling context, as well as the
provided value stored under the provided key.

=head2 delete

    $new_context = $context->delete($key)

Takes a key object (see the L</key> class method) and returns a new context
object with all the values of the calling context, except the value stored
